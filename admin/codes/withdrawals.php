...
if (isset($_POST['withdraw'])) {
    ...
    // Note: Assuming amount is in USD, as per frontend. Add rate if conversion is needed.
    $total = $amount;
...
if (isset($_POST['complete'])) {
    ...
    // Use prepared statement to update withdrawal status to completed
    $complete_query = "UPDATE withdrawals SET status = '1' WHERE id = ? LIMIT 1";
    $stmt = $con->prepare($complete_query);
    $stmt->bind_param("i", $id);

    if ($stmt->execute()) {
        $_SESSION['success'] = "Withdrawal request marked as completed.";
        header("Location: ../admin/pending_withdrawals.php");
        exit(0);
    } else {
        $_SESSION['error'] = "Failed to mark withdrawal request as completed.";
        header("Location: ../admin/pending_withdrawals.php");
        exit(0);
    }
    $stmt->close();
